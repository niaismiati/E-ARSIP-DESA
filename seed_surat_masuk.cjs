...
suratData.forEach(data => {
  db.prepare('INSERT INTO surat_masuk (nomor_surat, perihal, asal_surat, klasifikasi_id, tanggal_surat, tanggal_terima, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)').run(
    data.nomor_surat, data.perihal, data.asal_surat, data.klasifikasi_id, data.tanggal_surat, data.tanggal_surat, data.status
  );
});

console.log('✅ Added 3 surat masuk with 2026 dates and klasifikasi');
console.log('Count 2026:', db.prepare('SELECT COUNT(*) as c FROM surat_masuk WHERE strftime(\'%Y\', tanggal_surat) = \'2026\'').get().c);
console.log('Laporan ready!');
